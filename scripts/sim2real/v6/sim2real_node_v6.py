#!/usr/bin/env python3
"""
Sim2Real Node for UR5e pose control via RTDE – **V6** (LSTM policy).

Same observation/action space as V3 (24-dim obs, 12-dim actions) but uses
an LSTM-based policy that maintains internal hidden state across time-steps.

Key difference from V3:
  - ``policy.reset()`` is called on startup, and whenever the goal changes,
    to clear the LSTM hidden/cell state.
  - The LSTM learns temporal context (delays, dynamics) without needing
...
3. The **control loop runs at 60 Hz**, builds 24-dim normalised
   observations, runs 12-dim LSTM policy inference
...
Usage:
    ...
    python3 sim2real_node_v6.py --robot gripper --model path/to/policy.pt
    python3 sim2real_node_v6.py --robot gripper --rate 60 --action-scale 0.3 --velocity-scale 0.7 --model path/to/policy.pt
"""
...
# Local imports – observation builder is identical to V3
from observation_builder_v6 import (
...
from policy_inference_v6 import load_policy
...
# Reuse the V3 RTDE config and URScript (same q_des + qdot_des interface)
...
# ============================================================================
# RTDE communication helper (V3/V6: q_des + qdot_des)
# ============================================================================
...
class RTDEController:
    ...
    Identical to V3 – the RTDE/URScript layer is unchanged.
    """
    ...
    def connect(self):
        ...
        self.connected = True
        print("[RTDE] Connected and synchronised (V6/LSTM – with qdot_des)")
...
# ============================================================================
# ROS2 + RTDE sim2real node (V6 – LSTM)
# ============================================================================

class Sim2RealNode(Node):
    """ROS2 node for sim2real LSTM policy deployment with velocity feedforward.

    Compared to V3:
      - The policy object is a ``PolicyInferenceLSTM`` that manages hidden state.
      - ``policy.reset()`` is called on startup and when the goal changes.
    """
    ...
    def __init__(
        ...
        robot_host: str = ROBOT_HOST,
        reset_on_goal_change: bool = True,
    ):
        super().__init__("sim2real_policy_node_v6")
        ...
        self.reset_on_goal_change = reset_on_goal_change
        ...
        # ==================================================================
        # Load LSTM policy (24-dim obs → 12-dim actions, stateful)
        # ==================================================================
        self.get_logger().info(f"[V6-LSTM] Loading policy from: {model_path or 'default'}")
        self.policy = load_policy(model_path=model_path, device=device)
        self.get_logger().info("[V6-LSTM] Policy loaded – resetting LSTM hidden state")
        self.policy.reset()

        # ==================================================================
        # RTDE controller (same as V3 – q_des + qdot_des)
        # ==================================================================
        ...
        self.get_logger().info("Connecting to robot via RTDE (V6/LSTM)...")
        self.rtde.connect()
        ...
        self.get_logger().info(f"Sim2Real V6 (LSTM) node initialised at {control_rate} Hz")
        self.get_logger().info(
            f"Action scale: {action_scale}  |  Velocity scale: {velocity_scale}  |  "
            f"Reset on goal change: {reset_on_goal_change}"
        )
    ...
    def goal_pose_callback(self, msg: PoseStamped):
        ...
        When the goal changes significantly, the LSTM hidden state is reset
        so the policy starts fresh for the new target.
        """
        with self.lock:
        ...
        # Reset LSTM hidden state when goal changes (outside the lock)
        if goal_changed and self.reset_on_goal_change:
            self.policy.reset()
            self.get_logger().info("[V6-LSTM] Hidden state reset (goal changed)")
    ...
    def control_loop(self):
        ...
        # 3. Build observation (24-dim, normalised – same as V3)
        ...
        # 4. LSTM policy inference (12-dim output, hidden state updated in-place)
    ...
    def start(self):
        ...
        self.policy.reset()  # Ensure fresh LSTM state on start
        self.get_logger().info("[V6-LSTM] Control loop started (hidden state reset)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Sim2Real V6 LSTM Policy Deployment (RTDE + velocity feedforward)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to TorchScript LSTM policy (.pt file)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-reset-on-goal", action="store_true", default=False,
        help="Disable LSTM reset when goal changes (keep temporal context)",
    )
    parser.add_argument(
    ...
    try:
        node = Sim2RealNode(
            ...
            reset_on_goal_change=not args.no_reset_on_goal,
        )
        ...
        if args.benchmark:
            ...
            out_path = str(REPO_ROOT / "logs" / "benchmarks" / f"policy_bench_v6_{ts}.json")
