...
def parse_env_config(env_config_path: str = "env.yaml") -> Dict[str, Any]:
    ...
    class SafeLoaderIgnoreUnknown(yaml.SafeLoader):
        def ignore_unknown(self, node):  # type: ignore[override]
            return None

        @staticmethod
        def tuple_constructor(loader, node):  # type: ignore[override]
            return tuple(loader.construct_sequence(node, deep=True))

        @staticmethod
        def construct_object_as_mapping(loader, tag_suffix, node):  # type: ignore[override]
            # Treat !!python/object:* as a plain mapping so we don't lose data.
            # deep=True ensures nested objects/tuples are also constructed
    ...
    SafeLoaderIgnoreUnknown.add_multi_constructor(
        "tag:yaml.org,2002:python/object", SafeLoaderIgnoreUnknown.construct_object_as_mapping
    )
    ...
    if data is None or not isinstance(data, dict):
        raise ValueError(f"Failed to parse env config at: {env_config_path}. Expected dict, got {type(data)}")
    return data
...
def get_robot_joint_properties(
    ...
    """
    gripper_robot = data.get("gripper_robot")
    if gripper_robot is None:
        raise ValueError("'gripper_robot' key not found in env config. Check YAML structure.")
    
    actuator_data = gripper_robot.get("actuators")
    if actuator_data is None:
        raise ValueError("'actuators' key not found in gripper_robot config. Check YAML structure.")
...
def get_physics_properties(data: dict) -> Tuple[int, float, int]:
    ...
    return (
        data.get("decimation", 1),
        sim_data.get("dt", 0.01),
        sim_data.get("render_interval", 1),
    )


def get_task_properties(data: dict) -> Tuple[float, float]:
    """Extract task-level scaling parameters from the env config.

    Args:
        data (dict): Parsed environment configuration data.

    Returns:
        tuple: (action_scale, dof_velocity_scale)
    """

    return float(data.get("action_scale", 1.0)), float(data.get("dof_velocity_scale", 1.0))