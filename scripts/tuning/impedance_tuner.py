...
`scripts/tuning/sim_gain_tuner_logger.py` so it can be read directly by
`scripts/tuning/plot_sim_gain_tuner_csv.py`.
...
def run_tuner(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print("[Tuner] Use scripts/tuning/plot_sim_gain_tuner_csv.py --file <csv_path> to generate the plots.")
