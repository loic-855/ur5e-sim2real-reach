...
class DomainRandomizationCfg:
    ...
    """Observation noise on TCP linear velocity (m/s)."""
    obs_noise_std_tcp_ang_vel: float = 0.02
    """Observation noise on TCP angular velocity (rad/s)."""
...
class ObservationBuffer:
    ...
    The noise standard-deviations are applied per-component for the v2 layout:

        to_target_norm (3), orientation_error_norm (3),
        joint_pos_norm (6), joint_vel_norm (6),
        tcp_linear_vel_norm (3), tcp_angular_vel_norm (3)

    All 24 components are normalised; noise stds should be tuned accordingly.
    """
    ...
    def _build_noise_vector(self) -> torch.Tensor:
        """Construct a 1-D noise-std tensor aligned with the v2 observation layout (24-dim)."""
        nj = self.num_joints
        cfg = self.cfg
        ...
        if self.obs_dim != expected_dim:
            raise ValueError(
                f"ObservationBuffer expects v2 layout dim={expected_dim}, got obs_dim={self.obs_dim}. "
                "Update environment observation_space/layout or this noise builder."
            )

        parts: list[torch.Tensor] = [
            torch.full((3,), cfg.obs_noise_std_pos, device=self.device),             # to_target_norm
            torch.full((3,), cfg.obs_noise_std_quat, device=self.device),            # orientation_error_norm
            ...
            torch.full((3,), cfg.obs_noise_std_tcp_lin_vel, device=self.device),     # tcp_linear_vel_norm
            torch.full((3,), cfg.obs_noise_std_tcp_ang_vel, device=self.device),     # tcp_angular_vel_norm
        ]
