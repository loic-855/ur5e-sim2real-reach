...
"""
Pose-and-orientation reaching task for a UR5e arm **with screwdriver** (sim-to-real variant V1).

Uses the SCREWDRIVER_TCP robot model (7 joints: 6 arm + 1 sliding joint).
The screwdriver is present in the simulation for visual/collision fidelity with
...
Screwdriver joint is held at its default position (retracted).
"""
...
NUM_ARM_JOINTS = 6
# Number of screwdriver joints
NUM_SCREWDRIVER_JOINTS = 1
...
@configclass
class PoseOrientationSim2RealScrewdriverV1Cfg(DirectRLEnvCfg):
    """Configuration for V1 – UR5e + screwdriver, only arm controlled, position-only actions."""
    ...
    # SCREWDRIVER_TCP robot model (8 joints: 6 arm + 1 sliding joint)
    robot = get_robot_cfg(RobotType.SCREWDRIVER_TCP, "/World/envs/env_.*/ur5e_screwdriver_tcp")
    ...
    frame_transformer = FrameTransformerCfg(
        prim_path="/World/envs/env_.*/ur5e_screwdriver_tcp/ur5e/base_link",
        ...
        target_frames=[
            FrameTransformerCfg.FrameCfg(
                prim_path="/World/envs/env_.*/ur5e_screwdriver_tcp/ur5e/wrist_3_link",
                name="ee_tcp",
                offset=OffsetCfg(
    ...
    contact_sensor: ContactSensorCfg = ContactSensorCfg(
        prim_path="/World/envs/env_.*/ur5e_screwdriver_tcp/ur5e/.*",
        update_period=0.0,
        history_length=6,
        debug_vis=False,
...
class PoseOrientationSim2RealScrewdriverV1(DirectRLEnv):
    """Pose + orientation reaching with position-only control – screwdriver present but passive."""

    cfg: PoseOrientationSim2RealScrewdriverV1Cfg
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._num_total_joints = self._robot.num_joints  # 7 for SCREWDRIVER_TCP
        ...
        # Full joint targets (7-dim) – arm + screwdriver.
        self.robot_dof_targets = self._robot.data.joint_pos.clone()  # (N, 7)

        # Default screwdriver position
        self._screwdriver_default_pos = self._robot.data.default_joint_pos[0, NUM_ARM_JOINTS:].clone()  # (1,)
        # Alias used by shared helper methods
        self._gripper_default_pos = self._screwdriver_default_pos
        ...
        if self.cfg.debug and self.common_step_counter % 100 == 0:
            try:
                print(
                    ...
                    f"Screwdriver targets: {self.robot_dof_targets[0, NUM_ARM_JOINTS:].cpu().numpy()}"
                )
    ...
    def _reset_to_home(self, env_ids: torch.Tensor):
        """Reset around the home pose (arm joints randomised, screwdriver at default)."""
        home = self._robot.data.default_joint_pos[env_ids]  # (N, 7)
    ...
    def _reset_random(self, env_ids: torch.Tensor):
        """Reset to a random arm pose within joint limits; screwdriver at default."""
