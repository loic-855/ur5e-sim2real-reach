...
"""
Pose-and-orientation reaching task for a UR5e arm (sim-to-real variant V5).

Extension of V3 with **observation stacking**: the policy receives the last
``obs_stack_frames`` observations concatenated, giving it a short temporal
context to implicitly estimate latencies and dynamics.

Single-frame observations (24-dim, same as V3):
    pos_error (3), ori_error (3), joint_pos (6), joint_vel (6),
    tcp_linear_vel (3), tcp_angular_vel (3).

Stacked observation (default 3 frames → 72-dim):
    [obs_t-2, obs_t-1, obs_t]  (oldest first)
...
# Single-frame observation dimensionality (same as V3)
_SINGLE_OBS_DIM = 24


@configclass
class PoseOrientationSim2RealV5Cfg(DirectRLEnvCfg):
    """Configuration for V5 – V3 + observation stacking."""
    ...
    """Number of consecutive observation frames to stack (oldest first)."""

    # spaces – 12 actions (same as V3); observation = single_obs × stack_frames
    action_space = 12
    observation_space = _SINGLE_OBS_DIM * 3  # 72  (updated in __init__ if obs_stack_frames differs)
    state_space = 0
...
class PoseOrientationSim2RealV5(DirectRLEnv):
    """Pose + orientation reaching with velocity feedforward and observation stacking."""

    cfg: PoseOrientationSim2RealV5Cfg
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Ensure observation_space matches obs_stack_frames before parent __init__
        cfg.observation_space = _SINGLE_OBS_DIM * cfg.obs_stack_frames
        super().__init__(cfg, render_mode, **kwargs)

        self.dt = self.cfg.sim.dt * self.cfg.decimation
        self._num_envs = self.scene.cfg.num_envs
        self._single_obs_dim = _SINGLE_OBS_DIM
        self._obs_stack_frames = self.cfg.obs_stack_frames
        ...
        # ===================================================================
        # Observation stacking buffer
        # Shape: (num_envs, obs_stack_frames, single_obs_dim)
        # Oldest frame at index 0, newest at index -1.
        # ===================================================================
        self._obs_history = torch.zeros(
            (self._num_envs, self._obs_stack_frames, self._single_obs_dim),
            device=self.device,
            dtype=torch.float32,
        )
        ...
        # DR ObservationBuffer operates on single-frame obs (24-dim), applied
        # *before* stacking so that each frame in the stack can have independent
        # noise/delay as on the real system.
        self._obs_buffer = ObservationBuffer(
            num_envs=self._num_envs,
            obs_dim=self._single_obs_dim,
            num_joints=6,
            cfg=self.cfg.domain_rand,
            device=self.device,
        )
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # --- Position increments (same logic as V3) ---
        increments = (
    ...
    def _compute_single_obs(self) -> torch.Tensor:
        """Compute the 24-dim single-frame observation (normalised)."""
    ...
    def _compute_single_obs_raw(self) -> torch.Tensor:
        """Compute the 24-dim single-frame observation (raw / unnormalised)."""
        frame_data = self._frame_transformer.data
        tcp_pos_source = frame_data.target_pos_source[:, self._ee_frame_idx, :]
        tcp_quat_source = frame_data.target_quat_source[:, self._ee_frame_idx, :]
    ...
    def _push_obs_and_stack(self, single_obs: torch.Tensor) -> torch.Tensor:
        """Shift the observation history and append the new frame.

        Returns the flattened stacked observation (num_envs, obs_stack_frames * single_obs_dim).
        """
        # Shift left: drop oldest, append newest
        self._obs_history[:, :-1, :] = self._obs_history[:, 1:, :].clone()
        self._obs_history[:, -1, :] = single_obs

        # Flatten: (num_envs, obs_stack_frames * single_obs_dim)
        return self._obs_history.reshape(self._num_envs, -1)
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        # Domain randomisation on single-frame obs (noise/delay) before stacking
        if self.cfg.domain_rand.enabled:
            single_obs = self._obs_buffer.append_and_get(single_obs)

        # Stack observations
        stacked_obs = self._push_obs_and_stack(single_obs)

        if self.cfg.debug and self.common_step_counter % 100 == 0:
            sample = stacked_obs[0].cpu().numpy()
            print(
                f"[V5] Stacked obs shape: {stacked_obs.shape}, "
                f"sample (first 24): {sample[:24]}, "
                f"sample (last 24): {sample[-24:]}"
            )
            mins = stacked_obs.min(dim=0).values.cpu().numpy()
            maxs = stacked_obs.max(dim=0).values.cpu().numpy()
            print(f"[V5] Stacked obs min: {mins}\n[V5] Stacked obs max: {maxs}")

        return {"policy": stacked_obs}

    def _get_observations_raw(self) -> dict[str, torch.Tensor]:
        """Return stacked raw (unnormalised) observations."""
        single_obs = self._compute_single_obs_raw()
        ...
        if self.cfg.debug and self.common_step_counter % 100 == 0:
            sample = stacked_obs[0].cpu().numpy()
            print(
                f"[V5 Raw] Stacked obs shape: {stacked_obs.shape}, "
                f"sample (first 24): {sample[:24]}, "
                f"sample (last 24): {sample[-24:]}"
            )
